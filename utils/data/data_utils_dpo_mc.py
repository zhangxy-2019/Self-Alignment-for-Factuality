...
class QueryDataset(Dataset):
    ...
    def __init__(
        ...
        max_seq_len: int,
        num_negative_example: int,
        pad_token_id: int
    ):
        super(QueryDataset, self).__init__()
        
        df = get_stack_exchange_paired(
            df,
            tokenizer,
            max_seq_len,
            num_negative_example
        )
        self.queries = [item["prompt"] for item in df]
        self.chosen = [item["chosen"] for item in df]
        self.rejected = [item["rejected"] for item in df]
    ...
    def __getitem__(self, i):
        ...
        return dict(prompt=self.queries[i], chosen=self.chosen[i], rejected=self.rejected[i], pad_token_id=self.pad_token_id)
...
def get_stack_exchange_paired(
    ...
    for prompt in prompts:
        while len(tokenizer.tokenize(prompt)) + 2 > max_seq_len: # bos, ans, end token
            prompt_split = prompt.split("\n\n")
            prompt_split.pop(1)
            prompt = '\n\n'.join(prompt_split)
        queries.append(prompt) 

    a_probs = [item["a_probs"] for item in df]
    ans_lists = [item["ans_list"] for item in df]
    top_ans_list = []
    top_ans_text_list = []
    for i, a_prob in enumerate(a_probs):
        sorted_prob = a_prob.copy()
        sorted_prob.sort(reverse=True) 
        top_ans = []
        top_ans_text = []
        # print("sorted_prob: ", sorted_prob)
        for prob_k in sorted_prob:
            # prob_k_id = item["a_probs"].index(prob_k)
            prob_k_id = a_prob.index(prob_k)
            top_ans.append(choices[prob_k_id])
            top_ans_text.append(ans_lists[i][prob_k_id])
        assert len(top_ans) == len(a_prob) == len(top_ans_text)
        top_ans_list.append(top_ans)
        top_ans_text_list.append(top_ans_text)

    assert len(queries) == len(top_ans_list) == len(top_ans_text_list)
    final_df = []
    for i, (query, top_ans_list, top_ans_text_list) in enumerate(zip(queries, top_ans_list, top_ans_text_list)):
    # for i, query_item in enumerate(queries):
        print("top_ans_list: ", top_ans_list)
        pos_responses = top_ans_list[0]
        # pos_text = pos_responses + ". " + top_ans_text_list[0]
        for j, (neg_res, neg_text) in enumerate(zip(top_ans_list[1:], top_ans_text_list[1:])):
            if neg_res != pos_responses:
                final_df.append({"prompt":query, "chosen": pos_responses, "rejected": neg_res})
    # for i, (query, top_ans_list, top_ans_text_list) in enumerate(zip(queries, top_ans_list, top_ans_text_list)):
    # # for i, query_item in enumerate(queries):
    #     print("top_ans_list: ", top_ans_list)
    #     pos_responses = top_ans_list[0]
    ...
    #     for j, (neg_res, neg_text) in enumerate(zip(top_ans_list[1:], top_ans_text_list[1:])):
    #         if neg_res != pos_responses:
    #             final_df.append({"prompt":query, "chosen": pos_text, "rejected": neg_res + ". " + neg_text})
    print("final_df:\n", final_df[:2])
...
def create_prompt_dataset(local_rank,
    ...
    fname = f"{fname}_phase{train_phase}_seed{seed}_tokenizer{tokenizer_name}_seqlen{max_seq_len}_sft{sft_cache_key}_dpo_new"
    ...
    if local_rank <= 0 and (buf_create_cache.item() != 0 or reload):
        ...
        train_dataset = QueryDataset(
            ...
            num_negative_example=num_negative_example,
