...
class UnsupervisedtfDataset(Dataset):
    ...
    def __init__(
        ...
        labels = copy.deepcopy(query_ids)
        for label in labels:
            label[: (length -1)] = -1
    ...
    def __getitem__(self, i):
        return dict(queries=self.queries[i], query_attn_masks=self.query_attn_masks[i], labels=self.labels[i], pad_token_id=self.pad_token_id)
...
class DataCollatorUnsupervised:
    ...
    def __call__(self, data):
        ...
        batch["labels"] = torch.tensor([f["labels"].numpy().tolist()[0] for f in data])
