...
def get_stack_exchange_paired(
    ...
    for i, a_prob in enumerate(a_probs):
        ...
        assert len(top_ans) == len(a_prob)
        top_ans_list.append(top_ans)

    assert len(queries) == len(top_ans_list)
    final_df = []
    for i, query_item in enumerate(queries):
        pos_responses = top_ans_list[i][0]
        for neg_res in top_ans_list[i][1:]:
            if neg_res != pos_responses:
                final_df.append({"prompt":query_item, "chosen": pos_responses, "rejected": neg_res})
    print("final_df:\n", final_df)
