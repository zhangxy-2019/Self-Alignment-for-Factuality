...
PROMPT_DICT = {
    "prompt_input": (
        "### Instruction:\nPlease evaluate the truthfulness of the proposed answer based on the question and internal knowledge.\n\n### {text}"
    ),
    # "prompt_no_input": (
...
class UnsupervisedtfDataset(Dataset):
    ...
    def __init__(
        ...
        df = df["train"]
        if instruction:
            prompts = [PROMPT_DICT["prompt_no_input"].format_map(item) for item in df]
        ...
        for i, prompt_text in enumerate(prompts):
            ...
            if few_shot: 
                while len(tokenizer.tokenize(prompt)) + 1 > max_seq_len: # bos, end token
                    ...
                    prompt = '\n\nQuestion:'.join(prompt_split_shu)
            else:
                prompt = "Question:" + prompt.split("\n\nQuestion:")[-1]
                
            # if instruction:
            #     prompt = "Please evaluate the truthfulness of the proposed answer based on the question and internal knowledge.\n" + prompt
