...
class QueryDataset(Dataset):
    ...
    def __init__(
        ...
        for prompt in prompts:
            # while len(tokenizer.tokenize(prompt)) + 1> max_seq_len: # bos token
    ...
    def __getitem__(self, i):
        ...
        return dict(queries=self.queries[i], query_attn_masks=self.query_attn_masks[i], pad_token_id=self.pad_token_id, mc_label=self.mc_label[i])
...
class UnsupervisedtfDataset(Dataset):
    ...
    def __init__(
        ...
        # df = df["train"]
        prompts_v0 = [item["text"] for item in df]
        prompts = []
        for pro in prompts_v0:
            pro = pro.replace("Is the proposed answer:\n (A) True\n (B) False\n", "Is the proposed answer:\n A. True\n B. False\n")
            pro = pro.replace("The proposed answer is:(A)", "The proposed answer is:A")
            pro = pro.replace("The proposed answer is:(B)", "The proposed answer is:B")
            # print("replaced prompt: ", pro)
            prompts.append(pro)

        mc_labels = [item["output"].replace("(A)", "A").replace("(B)", "B") for item in df]
        # print("mc labels: ", mc_labels)
        # print("few-shot prompts:\n", prompts)
        # print("len prompts:\n", len(prompts)) # 285
        queries = []
        for i, prompt_text in enumerate(prompts):
            prompt = prompt_text + " " + mc_labels[i]
            # print("prompt text:\n", prompt)
            while len(tokenizer.tokenize(prompt)) + 1 > max_seq_len: # bos, end token
                prompt_split = prompt.split("\n\nQuestion:")
                prompt_split_shu = random.shuffle(prompt_split[:-1]) + prompt_split[-1:]
                prompt_split_shu.pop(1)
...
def create_prompt_dataset(local_rank,
    ...
    if local_rank <= 0 and (buf_create_cache.item() != 0 or reload):
        extensions = "json"
        # print("data path: ", data_path) # /apdcephfs_cq2/share_1603164/user/svetzhang/cali_data0826/calibration_datasets/true_false
        data_files = [
            glob.glob(data_path + "/" + fn + "/*.json")[0] for fn in os.listdir(data_path) if "mmlu" not in fn
        ]
        # data_files = [
        #     fn for fn in os.listdir(data_path)
        # ] # ['truthfulqa', 'mmlu', 'logiqa', 'quality', 'bigbench']
        # print("data file path: ", data_files)
        all_df = []
        for data_path in data_files:
            ...
            all_df.append(train_df['train'])
        # print("len all df: ", len(all_df))
        #     # len all df:  4
        #     # concate dataset:  Dataset({
        #     #     features: ['output', 'text'],
        #     #     num_rows: 163466
        #     # })
        concatenated_dataset = concatenate_datasets(all_df)
        ...
        #     token=None,
        # )
        print("concate dataset: ", concatenated_dataset)
        train_dataset = UnsupervisedtfDataset(
            df=concatenated_dataset,
            tokenizer=tokenizer,
