...
def get_stack_exchange_paired(
    ...
    pos_res = [item["pos"] for item in df]
    neg_res = [item["neg"] for item in df]

    queries = []
    final_df = []
    for i, prompt in enumerate(prompts):
        if few_shot: 
            while len(tokenizer.tokenize(prompt)) + 2 > max_seq_len: # bos, end token
                prompt_split = prompt.split("\n\nQuestion:")
                prompt_split.pop(1)
                prompt = '\n\nQuestion:'.join(prompt_split)
        else:
            prompt_instruction = prompt.split("\n\nQuestion:")[0]
            prompt_question = "Question:" + prompt.split("\n\nQuestion:")[-1]
            if instruction:
    ...
    for i, (query, pos_res, neg_res) in enumerate(zip(queries, pos_res, neg_res)):
        final_df.append({"prompt":query, "chosen": pos_res, "rejected": neg_res})
    print("final df:\n", final_df)
