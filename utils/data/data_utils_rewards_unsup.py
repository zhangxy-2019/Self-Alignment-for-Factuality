...
class QueryDataset(Dataset):
    ...
    def __init__(
        ...
        for prompt in prompts:
            while len(tokenizer.tokenize(prompt)) + 2> max_seq_len: # bos token
        ...
        self.pred_answers = [item["pred_answers"] for item in df]
        self.answer_probs = [item["answer_probs"] for item in df]
        self.pad_token_id = pad_token_id
        # self.list_dict_data = list_dict_data

    def __getitem__(self, i):
        return dict(queries=self.queries[i], query_attn_masks=self.query_attn_masks[i], pad_token_id=self.pad_token_id, mc_label=self.mc_label[i], pred_answers=self.pred_answers[i], answer_probs=self.answer_probs[i])
...
def create_prompt_dataset(local_rank,
    ...
    fname = f"{fname}_phase{train_phase}_seed{seed}_tokenizer{tokenizer_name}_seqlen{max_seq_len}_sft{sft_cache_key}_rewards_unsup"
...
class DataCollatorRLHF:
    ...
    def __call__(self, data):
        batch = {}
        pred_answers = [f["pred_answers"] for f in data]
        # answer_probs = [f["answer_probs"] for f in data]
        prompt_mclabel = [f["mc_label"] for f in data]
        prompt_answer_probs = [f["answer_probs"] for f in data]
        ...
        batch["prompt_pred_answers"] = pred_answers
        batch["prompt_answer_probs"] = prompt_answer_probs
        # batch["prompt_mclabel"] = prompt_mclabel
