...
class QueryDataset(Dataset):
    ...
    def __init__(
        ...
        self.pred_probs = [item["pred_probs"] for item in df]
        self.pred_ans = [item["pred_ans"] for item in df]
        self.pad_token_id = pad_token_id
        # self.list_dict_data = list_dict_data

    def __getitem__(self, i):
        return dict(queries=self.queries[i], query_attn_masks=self.query_attn_masks[i], pad_token_id=self.pad_token_id, pred_probs=self.pred_probs[i], pred_ans=self.pred_ans[i])
...
class DataCollatorRLHF:
    ...
    def __call__(self, data):
        ...
        batch["prompt_pred_probs"] = [f["pred_probs"] for f in data]
        batch["prompt_pred_ans"] = [f["pred_ans"] for f in data]
        # return dict(queries=self.queries[i], query_attn_masks=self.query_attn_masks[i], pad_token_id=self.pad_token_id, pred_probs=self.pred_probs[i], pred_ans=self.pred_ans[i])
