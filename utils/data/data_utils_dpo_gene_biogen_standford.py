...
def get_stack_exchange_paired(
    ...
    prompts = [item["input"] for item in df]
    generations = [item["ranked_generations"] for item in df]
    ranked_probs = [item["ranked_pred_probs"] for item in df]
    queries = []
    final_df = []
    ...
    final_df = []
    for i, (query, item_generations, item_ranked_probs) in enumerate(zip(queries, generations, ranked_probs)):
        for j, (gen, prob) in enumerate(zip(item_generations, item_ranked_probs)):
            # print("gen: ", gen)
            # print("prob: ", prob)
            for k in range(j + 1, len(item_generations)):
                pos_ans = gen
                pos_prob = prob
                neg_ans = item_generations[k]
                neg_prob = item_ranked_probs[k]
                if neg_ans == pos_ans:
                    continue
                if neg_prob == pos_prob:
                    continue
                final_df.append({"prompt":query, "chosen": pos_ans, "rejected": neg_ans})
