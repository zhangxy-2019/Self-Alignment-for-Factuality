...
class UnsupervisedDataset(Dataset):
    ...
    def __init__(
        ...
        for i, prompt_text in enumerate(prompts):
            prompt = prompt_text + " " + mc_labels[i] + end_of_conversation_token
            # print("prompt text:\n", prompt)
            ...
            #     prompt_token[key_word] = y
            # print("prompt token input ids:\n", prompt_token["input_ids"]) # tensor([[    0,     0,     0,  ...,   726, 29989, 29958]])
            # print("prompt token input id size:\n", prompt_token["input_ids"].size()) # torch.Size([1, 2048])
            # print("prompt token attention mask:\n", prompt_token["attention_mask"]) # tensor([[0, 0, 0,  ..., 1, 1, 1]])
            # print("prompt token attention mask size:\n", prompt_token["attention_mask"].size()) # torch.Size([1, 2048])
            queries.append(prompt_token)
        query_ids = [query["input_ids"] for query in queries]
        query_attn_masks = [query["attention_mask"] for query in queries]
...
def create_prompt_dataset(local_rank,
    ...
    fname = f"{fname}_unsupervised_phase{train_phase}_seed{seed}_tokenizer{tokenizer_name}_seqlen{max_seq_len}_sft{sft_cache_key}_val"
...
class DataCollatorRLHF:
    ...
    def __call__(self, data):
        batch = {}
        # pad_token_id = data[-1]["pad_token_id"]
        # pad_token_id = data[0]["pad_token_id"]
        # print("pad_token_id:\n", pad_token_id)
