...
from dpo_trainer_v3 import DeepSpeedPPOTrainer, DeepSpeedPPOTrainerUnsupervised
from rlhf_engine import DeepSpeedRLHFEngine
from transformers.models.llama import LlamaTokenizer
from utils_dpo import DPODataCollatorWithPadding
import sys

sys.path.append(
    os.path.abspath(os.path.join(os.path.dirname(__file__), os.path.pardir)))
from utils.data.data_utils_dpo_mc import create_prompt_dataset, MiniDataset, DataCollatorRLHF, get_unsupervised_data
...
from utils.perf import print_throughput_step3_nocritic
...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=1,
        help=
        "num negative examples = 1,2,3 for mc_task."
    )
...
def create_datasets(args, tokenizer, train_phase=3):
    ...
    prompt_train_dataset = create_prompt_dataset(
        args.local_rank, args.data_path, args.data_eval_path,
        args.data_output_path, train_phase, args.seed, tokenizer,
        args.max_prompt_seq_len, args.num_negative_example)
    ...
    data_collator = DPODataCollatorWithPadding(tokenizer=tokenizer, max_length=args.max_prompt_seq_len)
...
def main():
    ...
    #                               fast_tokenizer=True)
    # print ("args.actor_model_name_or_path",args.actor_model_name_or_path)
    # load_hf_tokenizer will get the correct tokenizer and set padding tokens based on the model family
    # tokenizer = load_hf_tokenizer(args.actor_model_name_or_path,
    #                               fast_tokenizer=True)
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, (batch_prompt, batch_unsupervised) in enumerate(
            ...
            #                                   batch_prompt["prompt_pred_answers"],
            #                                 #   batch_prompt["prompt_mclabel"],
            #                                   batch_prompt["prompt_answer_probs"],
            #                                   args.max_prompt_seq_len,
            #                                   step,
            #                                   global_steps) 
            ...
            exp_dataset = exp_mini_dataset.add(batch_prompt)
            ...
            if exp_dataset is not None:
                ...
                for ppo_ep in range(args.ppo_epochs):
                    for i, (exp_data, unsup_data) in enumerate(
                        ...
                        actor_loss, critic_loss = trainer.train_ppo(exp_data)
                ...
                print_rank_0(
                    f'Epoch: {epoch} | Global Step: {global_steps} | Step: {step} | PPO Epoch: {ppo_ep+1} | Actor Loss: {actor_loss_sum/inner_iter} | Critic Loss: {critic_loss_sum/inner_iter} | Unsupervised Loss: {unsup_loss_sum/inner_iter}',
                    args.global_rank)
                ...
                #                        args.global_rank)
                # print_throughput_step3_nocritic(rlhf_engine.actor.module,
                #                             args, e2e_time,
                #                             trainer.generate_time, training_time,
                #                             args.global_rank)
        ...
        #     args.output_dir = os.path.join(args.output_dir, '{}-epoch{}'.format(checkpoint_prefix, epoch))
        ...
        #     args.output_dir = out_dir_ori
            ...
            if args.output_dir is not None and args.save_steps > 0 and global_steps % args.save_steps == 0:
            # if args.output_dir is not None:
                checkpoint_prefix = 'checkpoint'
                out_dir_ori = args.output_dir
                # args.output_dir = os.path.join(args.output_dir, '{}-epoch{}'.format(checkpoint_prefix, epoch))
                args.output_dir = os.path.join(args.output_dir, '{}-step{}'.format(checkpoint_prefix, global_steps))
                print_rank_0('saving model ...')
                rlhf_engine.actor = convert_lora_to_linear_layer(rlhf_engine.actor)
                # rlhf_engine.critic = convert_lora_to_linear_layer(rlhf_engine.critic)
                if args.enable_ema:
