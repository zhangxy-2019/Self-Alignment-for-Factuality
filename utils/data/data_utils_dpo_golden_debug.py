...
def get_stack_exchange_paired(
    ...
    pos_res = [item["mc_label"] for item in df]
    
    final_df = []
    for i, query_item in enumerate(queries):
        pos_responses = pos_res[i]
        neg_responses = [item for item in choices if item != pos_responses]
        assert len(neg_responses) == 3
        # neg_responses = random.choice(neg_responses)
        for neg_res in neg_responses:
            final_df.append({"prompt":query_item, "chosen": pos_responses, "rejected": neg_res})
            
        # neg_responses = top_ans_list[i][1:]
        # for n_res in neg_responses:
        #     final_df.append({"prompt":query_item, "chosen": pos_responses, "rejected": n_res})

    return final_df
    # dataset = Dataset.from_list(final_df)
