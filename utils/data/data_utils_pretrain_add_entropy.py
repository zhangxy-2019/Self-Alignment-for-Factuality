...
class UnsupervisedtfDataset(Dataset):
    ...
    def __init__(
        ...
        for i, prompt_text in enumerate(prompts):
            if instruction:
                prompt = "Instruction: Please evaluate the truthfulness of the proposed answer based on the question and internal knowledge.\n\n" 
            # else:
            #     prompt = "### Instruction:\n"
            prompt += prompt_text.strip() + mc_labels[i].strip()
            # prompt += "\n\n### Response:" + mc_labels[i].strip()
            ...
            if few_shot: 
                ...
                if instruction:
                    prompt_text = "Instruction: Please evaluate the truthfulness of the proposed answer based on the question and internal knowledge.\n\n" 
                # else:
            ...
            act_index = prompt_token["input_ids"].size()[-1] -1
            action_index.append(act_index)
            ### padding
    ...
    def __getitem__(self, i):
        return dict(queries=self.queries[i], query_attn_masks=self.query_attn_masks[i], action_indexs=self.action_index[i], pad_token_id=self.pad_token_id)
...
class DataCollatorUnsupervised:
    ...
    def __call__(self, data):
        ...
        batch["action_index"] = [f["action_indexs"] for f in data]
        return batch
