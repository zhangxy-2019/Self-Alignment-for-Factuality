...
def get_stack_exchange_paired(
    ...
    pos_res = [item["true_ans"] for item in df]
    neg_res = [item["false_ans"] for item in df]

    queries = []
    final_df = []
    for i, prompt in enumerate(prompts):
        if few_shot: 
            while len(tokenizer.tokenize(prompt)) + 20 > max_seq_len: # bos, end token
                prompt_split = prompt.split("\n\nQ:")
                prompt_split.pop(1)
                prompt = '\n\nQ:'.join(prompt_split)
        else:
            prompt_instruction = prompt.split("\n\nQ:")[0]
            prompt_question = "Q:" + prompt.split("\n\nQ:")[-1]
            if instruction:
                prompt_text = prompt_instruction + "\n\n" 
            else:
                prompt_text = ""
            prompt = prompt_text + prompt_question
        queries.append(prompt)

    assert len(queries) == len(neg_res) == len(pos_res)
    final_df = []
    for i, (query, pos_res, neg_res) in enumerate(zip(queries, pos_res, neg_res)):
        pos_res = list(set(pos_res))
        neg_res = list(set(neg_res))
        for pos_res_ in pos_res:
            for neg_res_ in neg_res:
                final_df.append({"prompt":query, "chosen": pos_res_, "rejected": neg_res_})
